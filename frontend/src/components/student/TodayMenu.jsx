...
const TodayMenu = ({ menu, timings }) => {
  if (!menu) return <div className="card shadow-sm border-0 mb-4 h-100 p-4 text-center">No menu available for today.</div>;
  ...
  return (
    <div className="card shadow-sm border-0 mb-4 h-100">
      <div className="card-body">
        <h5 className="card-title fw-bold text-primary mb-4">Today's Menu</h5>
        <div className="list-group list-group-flush">
          {mealItems.map(meal => (
            <div key={meal.type} className="list-group-item px-0 border-0 mb-3">
              <div className="d-flex w-100 justify-content-between align-items-center mb-1">
                <h6 className="mb-0 fw-bold">{meal.type}</h6>
                {meal.data?.extra && <span className="badge bg-warning text-dark small">Extra: {meal.data.extra}</span>}
              </div>
              {meal.time && (
